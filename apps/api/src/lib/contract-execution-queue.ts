 ...
 */
export async function queueContractExecution(contractId: string): Promise<void> {
  // Use setImmediate to execute in the next iteration of the event loop
  // This ensures the HTTP response is sent before execution starts
  setImmediate(async () => {
    try {
      console.log(`[Contract Execution Queue] Starting async execution for contract ${contractId}`);
 ...
 */
export async function shouldTriggerExecution(contractId: string): Promise<boolean> {
  try {
    ...
    console.error('Error checking if contract should trigger execution:', error);
    return false;
  }
}
