...
export const auth = betterAuth({
  ...
  socialProviders: {
    discord: {
      ...
      redirectUri: `${process.env.API_BASE_URL || 'http://localhost:3000'}/health` as string,
      ...
      // Map Discord profile -> Prisma User so required fields are set
      mapProfileToUser: (profile: any) => {
        return {
          discordId: profile.id,
          username:
            profile.global_name ?? profile.username ?? profile.name ?? '',
        };
      },
    },
  },
