...
import { parseArgs, showScriptHelp, setupEnvironment, printStatus, printSuccess, printError, executeCommand } from './utils.mjs';
...
function encryptDev() {
  ...
  const result = executeCommand('dotenvx encrypt .env.development.local -f .env.development');
...
function encryptProd() {
  ...
  const result = executeCommand('dotenvx encrypt .env.production.local -f .env.production');
...
function decryptDev() {
  ...
  const result = executeCommand('dotenvx decrypt .env.development -f .env.development.local');
  
  if (result.success) {
    printSuccess('Development environment decrypted successfully!');
    printStatus('Edit .env.development.local and then run encrypt-dev to save changes');
  } else {
    printError('Failed to decrypt development environment');
    printStatus('Make sure you have the correct DOTENV_PRIVATE_KEY_DEV in your .env.keys file');
    process.exit(1);
  }
}
...
function decryptProd() {
  ...
  const result = executeCommand('dotenvx decrypt .env.production -f .env.production.local');
  
  if (result.success) {
    printSuccess('Production environment decrypted successfully!');
    printStatus('Edit .env.production.local and then run encrypt-prod to save changes');
  } else {
    printError('Failed to decrypt production environment');
    printStatus('Make sure you have the correct DOTENV_PRIVATE_KEY_PRODUCTION in your .env.keys file');
    process.exit(1);
  }
}
