...
**Important**: Commands do NOT use generated event IDs. Instead, commands use `eventId` **provided by the sender** (API Gateway) in the protobuf message. The sender generates a unique `eventId` for each command to enable safe retries across network failures.

```typescript
// Commands: Use eventId from protobuf message (provided by sender)
const eventId = command.eventId; // From protobuf message, not generated

// Check idempotency using the provided eventId
const existing = await checkCommandIdempotency(prisma, eventId);
```

**Rationale**:

- Commands are external requests that can be retried by the sender
- The sender (API Gateway) generates a unique `eventId` for each command attempt
- This allows safe retries: if a command fails, the sender can retry with the same `eventId`
- The receiver checks idempotency using this `eventId` to prevent duplicate processing

#### Command ID vs Domain Event ID Strategy

**Two Different Approaches for Two Different Purposes**:

| Aspect          | Command IDs                                 | Domain Event IDs                   |
| --------------- | ------------------------------------------- | ---------------------------------- |
| **Source**      | Provided by sender (API Gateway)            | Generated by receiver (Service)    |
| **Method**      | `command.eventId` from protobuf             | `generateEventId()` function       |
| **Purpose**     | Enable safe retries across network failures | Ensure idempotent event processing |
| **Determinism** | Random UUID per request                     | Deterministic from business state  |
| **Storage**     | `CommandIdempotency` table                  | `EscrowEvent.payload` JSON field   |
| **Use Case**    | External retry safety                       | Internal consistency               |

**Why Different Approaches?**

- **Commands**: External requests that can fail due to network issues. The sender (API Gateway) generates a unique `eventId` and can safely retry with the same ID if the request fails.
- **Domain Events**: Internal state changes that must be idempotent. Services generate deterministic IDs from business state to ensure the same business change always produces the same event ID.

**Example**:

```typescript
// COMMAND: eventId provided by sender
const createEscrowCommand = {
  eventId: 'cmd_abc123', // Generated by API Gateway
  escrowId: 'esc_xyz',
  // ... other fields
};

// DOMAIN EVENT: eventId generated by service
const businessState = JSON.stringify({ amount: 10000, currency: 'USD' });
...
// eventId is deterministic - same inputs always produce same ID
```
...
**Important**: Commands use `eventId` **provided by the sender** (API Gateway) in the protobuf message, not generated IDs. The sender generates a unique `eventId` for each command to enable safe retries.
...
async function handleCommand(command: CreateEscrowCommand) {
  // eventId comes from protobuf message (provided by API Gateway)
  // Do NOT generate it - use the one from the command
  ...
  if (existing && existing.status === 'failed') {
    throw new Error(`Command previously failed: ${existing.error}`);
  }

  // Mark as pending before processing
  await storeCommandIdempotency(prisma, eventId, {
...
- **Commands**: Use `eventId` from protobuf message (provided by sender, not generated)
- **Webhooks**: Use `checkWebhookEventIdempotency()` with provider event ID
- **Domain Events**: Use `generateEventId()` and `checkEscrowEventIdempotency()` for deterministic IDs based on business state
...
async function handleCreateExampleCommand(command: CreateExampleCommand) {
  // eventId comes from protobuf message (provided by sender)
  // Do NOT generate it - use the one from the command
  const eventId = command.eventId;

  // Check idempotency using provided eventId
  const existing = await checkCommandIdempotency(prisma, eventId);

  if (existing && existing.status === 'completed') {
    console.log(`Command ${eventId} already processed`);
    return;
  }
  ...
  try {
    ...
    await prisma.$transaction(async tx => {
      ...
      // Generate business state hash for domain event ID
      const businessState = JSON.stringify({
        name: example.name,
      ...
      // Generate domain event ID (deterministic)
      const domainEventId = generateEventId(
        example.id,
        'ExampleCreated',
        businessStateHash,
        new Date()
      ...
      await createEscrowEventIdempotent(tx, {
        ...
        payload: {
          eventId: domainEventId, // Domain event ID
    ...
    await storeCommandIdempotency(prisma, eventId, {
      ...
      escrowId: command.exampleId,
      status: 'completed',
      result: { exampleId: command.exampleId },
    });
  } catch (error) {
...
4. **Idempotency**: Commands use `eventId` from protobuf messages, domain events use `generateEventId()` for deterministic generation
...
describe('Event ID Generation', () => {
  it('should generate deterministic IDs for domain events', () => {
    const businessState = JSON.stringify({ amount: 10000, currency: 'USD' });
    const businessStateHash = createHash('sha256')
      .update(businessState)
      .digest('hex');
...
await consumer.run(async message => {
  ...
  // Note: eventId comes from protobuf message, not generated here
  const eventId = message.value.eventId; // Extract from protobuf command
  const existing = await checkIdempotency(eventId);
